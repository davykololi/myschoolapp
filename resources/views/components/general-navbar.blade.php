<header class='font-[sans-serif] min-h-[60px] sticky top-0 z-10'>
    <section class="bg-[#004d66] min-h-[40px] px-4 py-2 sm:px-10 flex items-center max-sm:flex-col">
      ...
      <div class="sm:ml-auto text-white">
        <a href="{{ route('login') }}" class="text-white text-sm mr-1">Sign In</a>
      </div>
      @endif
      @else
      <div class="sm:ml-auto text-white">
        <form method="POST" action="{{ route('logout') }}">
              @csrf
              <a :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();" class="block px-4 py-2 text-sm hover:bg-black dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white cursor-pointer">
                Sign out
              </a>
            </form>
      </div>
    ...
    <div class='flex flex-wrap items-center justify-between py-3 px-4 sm:px-10 bg-[#151d20] lg:gap-y-4 gap-y-6 gap-x-4'>
      <a href="/" class="text-white flex inline-flex items-center">
        <x-graduates-cap/>
        <span class="ml-4 uppercase font-bold text-white">{{ config('app.name') }}</span>
      </a>
      ...
      <ul id="collapseMenu" class='lg:!flex lg:space-x-10 max-lg:space-y-2 max-lg:hidden max-lg:w-full max-lg:my-4'>
        <li
          ...
          <a href="{{ url('/') }}" class='text-white block text-[15px] font-medium'>Home</a>
        </li>
        <li
