...
class ThreeExamsReportCardPdfController extends Controller
{
    /**
    ...
    public function threeExamsReportCard(Request $request)
    {
        ...
        if((is_null($yearId)) || (is_null($termId)) || (is_null($streamId)) || (is_null($name)) || (is_null(auth()->user()->school->image)) || (is_null($closingDate)) || (is_null($openingDate))){
            return back()->withErrors(ucwords('Please ensure you have filled all the required details before you proceed!'));
        }
        ...
        if($values->doesntContain(ucwords($name))){
            return back()->withErrors(ucwords($name)." "."does not belong to"." ".$stream->name.". "."Please choose the correct stream and proceed!");
        }   
        ...
        $historyAndGovernmentAvg = Stat::mean([$examOneMark->history_and_government,$examTwoMark->history_and_government,$examThreeMark->history_and_government]);
        ...
        $historyAndGovernmentSubjectRemarks = historyAndGovernmentSubjectRemarks($historyAndGovernmentAvg,$yearId,$termId,$streamId);
        ...
        $totals = collect(session()->get("classTotal"));
        ...
        $stTotals = collect(session()->get("streamTotal"));
        ...
        $examThreeHistoryAndGovernmentGradePoints = examThreeHistoryAndGovernmentGradePoints($examThreeMark,$markName);
        $historyAndGovernmentCumulativePoints = Stat::mean([$examOneHistoryAndGovernmentGradePoints,$examTwoHistoryAndGovernmentGradePoints,$examThreeHistoryAndGovernmentGradePoints]);
        //Get Geog GPA 
        ...
        $pdf = PDF::loadView('admin.pdf.threeexams_reportcard',$data)->setOptions(['dpi'=>150,'defaultFont'=>'sans-serif','isRemoteEnabled' => true,'isHtml5ParserEnabled' => true])->setPaper('a4','potrait');
