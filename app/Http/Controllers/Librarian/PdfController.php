...
class PdfController extends Controller
{
    ...
    public function libraryBooks(School $school)
    {
    	...
    	$title = 'Library Books';
    	$pdf = PDF::loadView('librarian.library.library_books',['school'=>$school,'books'=>$books,'title'=>$title])->setOptions(['dpi'=>150,'defaultFont'=>'sans-serif','isRemoteEnabled' => true,'isHtml5ParserEnabled' => true])->setPaper('a4','landscape');
        $pdf->output();
    ...
    public function issuedBooksByReturnDate(Request $request)
    {
        ...
        $issuedBooks = Issuedbook::where('return_date',$returnDate)->get();
        $title = 'Library Books To Be Returned On'." ".Carbon::parse($returnDate)->format('d/m/Y');
        $pdf = PDF::loadView('librarian.library.issuedbooks_bydate',['issuedBooks'=>$issuedBooks,'title'=>$title,'school'=>$school])->setOptions(['dpi'=>150,'defaultFont'=>'sans-serif','isRemoteEnabled' => true,'isHtml5ParserEnabled' => true])->setPaper('a4','landscape');
